...
void listRemoveAt(List *list, unsigned int index) {
	list->nrNodes--;
}

void *listGet(List *list, int i) {
	return list->current[i].pdata;
}
...
unsigned int listSize(const List *list) {
	return list->nrNodes;
}

unsigned int listReserved(const List *list) {
	return list->nrReserved;
}